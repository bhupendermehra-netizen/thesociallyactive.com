...
<body>
    ...
    @if(env("SITE_SETTING"))
        ...
        <div class="loader">
            <video autoplay loop muted playsinline class="loader-video" poster="{{(env('IMG_FETCH_URL') . 'uploaded_files/' . header_footer()['main_component'][2]->img)}}">
                <source src="{{(env('IMG_FETCH_URL') . 'uploaded_files/' . str_ireplace('.gif', '.mp4', header_footer()['main_component'][2]->img))}}" type="video/mp4">
                <source src="{{ asset('uploaded_files/' . str_ireplace('.gif', '.mp4', header_footer()['main_component'][2]->img)) }}" type="video/mp4">
            </video>
        </div>
