...
@if(env("SITE_SETTING"))
	<div class="side_contact_button">
		<button class="mainButton" type="button"><img src="{{(env('IMG_FETCH_URL').'uploaded_files/'.header_footer()['side_buttons'][0]->img)}}"></button>
		<div class="sideButtonDiv" data-selected="0">
			<button class="sideButton" type="button" onclick="window.location.href='{{header_footer()['side_buttons'][1]->link}}'"><i class="{{header_footer()['side_buttons'][1]->text}}"></i></button>
		<button class="sideButton" type="button" onclick="window.location.href='{{header_footer()['side_buttons'][2]->link}}'"><i class="{{header_footer()['side_buttons'][2]->text}}"></i></button>
		<button class="sideButton" type="button" onclick="window.location.href='{{header_footer()['side_buttons'][3]->link}}'"><i class="{{header_footer()['side_buttons'][3]->text}}"></i></button>
			<button class="sideButton" type="button" onclick="window.location.href='{{header_footer()['side_buttons'][4]->link}}'"><i class="{{header_footer()['side_buttons'][4]->text}}"></i></button>
...
<div class="loader">
    ...
    <header class="navbar-top">
        <div class="navbar">
            ...
            <div class="link">
                ...
                <div class="button">
                    <div class="button_before">
                    </div>
                    
                    <a class="let_talk_link" href="{{header_footer()['navbar'][1]->link}}">{{header_footer()['navbar'][1]->text}} <i class='fa fa-phone'></i>
                    </a>
                </div>
    ...
    <div class="navbar-links">
        <div class="inner">
            ...
            <p class="links"><a href="{{header_footer()['navbar'][2]->link}}">{{header_footer()['navbar'][2]->text}}</a></p>
			@endif
			@if($_SERVER['REQUEST_URI'] != "/".str_replace("/","",header_footer()['navbar'][3]->link))
            <p class="links"><a href="{{header_footer()['navbar'][3]->link}}">{{header_footer()['navbar'][3]->text}}</a></p>
			@endif
			@if($_SERVER['REQUEST_URI'] != "/".str_replace("/","",header_footer()['navbar'][4]->link))
            <p class="links"><a href="{{header_footer()['navbar'][4]->link}}">{{header_footer()['navbar'][4]->text}}</a></p>
			@endif
			
			@if($_SERVER['REQUEST_URI'] != "/".str_replace("/","",route('projects')))
            <p class="links"><a href="{{route('projects')}}">Projects</a></p>
			@endif
			
            <p class="links"><a href="#" class="service_dropdown">{{header_footer()['navbar'][5]->text}}<i class='fa fa-arrow-down'></i></a></p>
...
"Identity","Positioning","Vision","Legacy","Influence","Authority","Signature","Alignment","Authenticity","Differentiation","Perception","Purpose","Trust","Consistency","Transformation","Engagement","Viral","Reach","Trend","Relevance","Community","Aesthetic","Stories","UGC (User Generated Content)","Real-time","Relatable","Impact","Collab (Collaboration)","Creator","Credibility","Micro influencer","Resonance","Partnership","Audience","Niche","Campaign","Retention","ROI (Return on Investment)","Funnels","Conversions","Awareness","Visibility","Disruption","Innovation","Metrics","Bold","Elevate","Magnetic","Loud","Unforgettable",
	...
	// Class definition moved UP before setup() to fix "Cannot access before initialization"
	class Word{
		...
		function setup(){
			...
			roof = Bodies.rectangle(width/2,height,width,30,{
			isStatic:true,		  
			});
