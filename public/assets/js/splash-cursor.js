// Fluid Cursor Effect - Vanilla JS (converted from React SplashCursor)
function initSplashCursor(canvasId, options) {
  ...
  const config = Object.assign({
    ...
    PAUSED: false,
    BACK_COLOR: { r: 0.92, g: 0.91, b: 0.95 },
    TRANSPARENT: true,
  }, options || {});
  ...
  function pointerPrototype() {
    ...
    this.moved = false; this.color = [0, 0, 0];
  }
  ...
  function getUniforms(program) {
    const u = []; const c = gl.getProgramParameter(program, gl.ACTIVE_UNIFORMS);
  ...
  function updateKeywords() {
    const k = []; if (config.SHADING) k.push("SHADING");
  ...
  function drawDisplay(target) {
    const w = !target ? gl.drawingBufferWidth : target.width;
    const h = !target ? gl.drawingBufferHeight : target.height;
  ...
  function getResolution(res) {
    ...
    if(gl.drawingBufferWidth>gl.drawingBufferHeight)return{width:mx,height:mn};
    return{width:mn,height:mx};
  }
  ...
  document.body.addEventListener("mousemove", function firstMove(e) {
    const p = pointers[0];
    updateFrame();
...
document.addEventListener("DOMContentLoaded", function() {
  initSplashCursor("splash-canvas");
});
